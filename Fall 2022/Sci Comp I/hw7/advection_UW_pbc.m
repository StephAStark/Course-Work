function [h,k,error] = advection_UW_pbc(p,m)
...
for n = 1:nsteps
     tnp = tn + k;   % = t_{n+1}
     % We need to edit here for both of the edited functions to change the
     % method used. This one we will change to the upwind method.
     % Lax-Wendroff:
...
%error_table(tnp,error)
%error_loglog(tnp,error)
